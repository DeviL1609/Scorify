        ...
        <div class="home-banner">
          <div class="home-container5">
            ...
            <form action="index.php" method="POST">
              ...
              if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['emailclick'])) {
                  $email = $_POST['email'];
                  ...
                  if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                      try {
                          ...
                          $mail->Username   = 'piyushsingh4237@gmail.com';     // SMTP username
                          $mail->Password   = 'zpbyunulfrmehmwc';     // SMTP password
                          $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // Enable TLS encryption
                          $mail->Port       = 587;                 // TCP port to connect to
              
                          // Recipients
                          $mail->setFrom('piyushsingh4237@gmail.com', 'Piyush Singh');
                          $mail->addAddress($email); // Add a recipient
              
                          // Content
                          $mail->isHTML(true); // Set email format to HTML
                          $mail->Subject = "Thanks For Subscribing :) - Scorify";
                          $mail->Body    = "Thank you for subscribing to Scorify. You'll always receive the latest updates from us.";
              
                          // Send the email
                          $mail->send();
                          echo '<div id="subscribed">✅ Subscribed Successfully!</div>';
                      } catch (Exception $e) {
                          echo '<div id="not-sub">❌ Failed to send mail: ' . $mail->ErrorInfo . '</div>';
                      }
                  } else {
                      echo '<div id="not-sub">❌ ' . $email . ' is not a valid email!</div>';
                  }
              }
              ?>
